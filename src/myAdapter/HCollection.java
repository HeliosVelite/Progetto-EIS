 ...
 * represents a group of objects, known as its <i>elements</i>, that can be duplicated.
 * Some collections allow duplicate elements and others do not. Some are ordered and others unordered. The SDK does not provide any direct implementations of this interface: it provides implementations of more specific subinterfaces like Set and List. 
 ...
 *
 * <br>
 * All general-purpose <i>Collection</i> implementation classes (which typically implement <i>Collection</i> indirectly through one of its subinterfaces) should provide two "standard" constructors: a void (no arguments) constructor, which creates an empty collection, and a constructor with a single argument of type <i>Collection</i>, which creates a new collection with the same elements as its argument. In effect, the latter constructor allows the user to copy any collection, producing an equivalent collection of the desired implementation type. There is no way to enforce this convention (as interfaces cannot contain constructors) but all of the general-purpose <i>Collection</i> implementations in the Java platform libraries comply.
 * 
 * <br>
 ...
 * UnsupportedOperationException if this collection does not support the operation. If this is the case, these methods may, but are not required to, throw an UnsupportedOperationException if the invocation would have no effect on the collection. 
 * For example, invoking the {@link #addAll(HCollection)} method on an unmodifiable collection may, but is not required to, throw the exception if the collection to be added is empty.
 *
 *<p>Some collection implementations have restrictions on the elements that they may contain.  For example, some implementations prohibit null elements 
 ...
 * <i>NullPointerException</i> or <i>ClassCastException</i>.  Attempting to query the presence of an ineligible element may throw an exception,
 *  or it may simply return false; some implementations will exhibit the former behavior and some will exhibit the latter.  More generally, attempting an
 *  operation on an ineligible element whose completion would not result in the insertion of an ineligible element into the collection may throw an
 *   xception or it may succeed, at the option of the implementation. Such exceptions are marked as "optional" in the specification for this interface. </p>
 *@see HList
 */
public interface HCollection {
     ...
     * contains more than <i>Integer.MAX_VALUE</i> elements, returns
    ...
    /**
     * Returns <i>true</i> if this collection contains the specified element. More
     * formally, returns <i>true</i> if and only if this collection contains at
